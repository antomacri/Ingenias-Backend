...
SELECT ProductID AS Codigo, UCASE(ProductName) AS Descripcion, QuantityPerUnit AS Presentacion, UnitPrice AS PrecioUnitario
FROM products;
...
SELECT ProductID AS Codigo, UCASE(ProductName) AS Descripcion, REPLACE(QuantityPerUnit, 'boxes', 'cajas') AS Presentacion, UnitPrice AS PrecioUnitario
FROM products
